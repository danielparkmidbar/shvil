...
- **발행 라우트의 코스 대조 (2026-07-26).** `/certificates`·`/claims`의 `courseId`는
  `GET /courses`가 배포하는 목록과 대조한다 (`src/courses.ts` — 배포와 발행이 같은
  함수를 본다). 등록되지 않은 코스는 거부(`UNKNOWN_COURSE`, fail-closed). 이 대조가
  없으면 "1인 1코스 1회"가 "1인 1문자열 1회"가 되어 무한 발행이 열린다.
- **첫 접대 보너스도 수량 한정.** 등록 보너스와 같은 방식·같은 주입 경로
  (`AppOptions.firstHostingQuota`, 기본 500명분)이고 `GET /transparency/promo`에
  공시된다. 증빙 코인에는 금액 하한이 있다 — 권장 가격표의 가장 싼 접대
  (샤워 30 dSHV = 3 SHV).
