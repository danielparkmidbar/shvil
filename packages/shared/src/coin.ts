 ...
 * 사람에게 "위조"라고 말하면 안 된다. UI 문구·위폐 감지기 판정이 이 집합을 기준으로 갈린다.
 *
 * ★2026-07-27 정정: 예전 이 주석은 "수령은 여전히 fail-closed로 막는다"고 적혀
 * 있었으나 **더 이상 참이 아니다.** 지갑 수령 경로(receiveReview.ts)는 이 사유들을
 * 막지 않고 엔젤에게 보여 주고 결정하게 한다 — 헌법 제9조("수용 여부는 언제나
 * 엔젤의 결정")를 따르기 위해서다. 코드가 서로 반대로 말하고 있었다.
 *
