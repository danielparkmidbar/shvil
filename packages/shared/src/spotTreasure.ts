 ...
 *
 * ★현장 결속 없음(V-1, 적대적 검증): "벽 QR 스캔"은 spotId 취득 수단일 뿐 위치 증명이
 * 아니다 — spotId만 알면 원격 청구가 가능하다(GET /spot이 spotId·위치를 공개). 이는
 * 서버가 위치를 볼 수 없다는 헌법 제9조 제약의 직접 귀결이며, 방문 유인은 게임적 동기와
 * 선착순 희소성에 기댄다. 근본 완화(M9 몸-걸음 인증 결합)는 결정 대기(R-스팟-현장결속,
 * 몸인증_보물마이닝_설계 4장). fake-walk 예치 세탁 차단(V-3)은 서버 초과생성 탐지에서 한다.
 *
